...
for (i in 1:1) {
	...
	if( c==6 ){
		compaired <- c(group[1],group[2],group[3],group[4],group[5],group[6])
	}
	if( c==7 ){
		compaired <- c(group[1],group[2],group[3],group[4],group[5],group[6],group[7])
	}
	if( c==8 ){
		compaired <- c(group[1],group[2],group[3],group[4],group[5],group[6],group[7],group[8])
	}
...
####准备lefse输入文件
for (i in 1:ncol(data_map)) {
  data_t_m <- cbind(data_map[,i],otu_its_t)
  lefse=paste(outpath,"/GroupType_",colnames(data_map)[i],"_lefsel.txt", sep="")
write.table(data_t_m, lefse, sep = " ",quote = FALSE)
}
